...
## Interactive / video
- **Rustlings** — tracked in this repo at `./rustlings/`. Drive via `cd rustlings && rustlings watch`. Do **in lockstep** with the Book — finish Book §X.Y → run the matching exercise set → don't advance until it passes. Progress cursor lives in `rustlings/.rustlings-state.txt`, so a fresh clone resumes mid-curriculum. Upstream: https://github.com/rust-lang/rustlings.
